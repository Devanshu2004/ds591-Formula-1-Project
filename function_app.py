...
def run_silver(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Optional JSON body:
    {
      "year": "2024,2025",
      "session_type": "R",
      "race_location": "Italian Grand Prix"
    }
    """
    ...
    try:
        ...
        session_type  = req.params.get("session_type")  or body.get("session_type")  or "R"
...
def run_gold(req: func.HttpRequest) -> func.HttpResponse:
    """
    Runs the gold processing pipeline for one driver.
    POST /api/run_gold
    ...
    try:
        ...
        driver = (
            req.params.get("target_driver")
            or body.get("target_driver")
            or get_env("TARGET_DRIVER", "LEC")
        )
...
def run_social_media_pipeline(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Moves data from social_media_bronze.json -> social_media_silver.json
    """
...
def run_model(req: func.HttpRequest) -> func.HttpResponse:
    ...
    NOTE: This function exists for reference only.
    Model training is run locally via: python3 src/model.py
    Models are then uploaded to platinum/models/ on Azure automatically.
    """
